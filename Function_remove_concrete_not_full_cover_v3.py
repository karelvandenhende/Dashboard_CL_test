...
def C_paper_vectorized2_over_depth(xmax, mortar, h2, D1, D2_ini, D2_rep, trep, T, Cs, nmax, interval):
    som = Cs.copy()
    lambdas = search_lambda5_vect(mortar / np.power(D1, 0.5), h2 / np.power(D2_rep, 0.5), D1, D2_rep, nmax,
    ...
    for tt in range(xmax):
        Term2 = fn(np.tile(tt, (nmax, 1)), mortar, h2, np.tile(D1, (nmax, 1)), np.tile(D2_rep, (nmax, 1)),
                   lambdas.T)
        som = Cs + np.sum(BB / integral_term * Term2 * np.exp(-np.power(lambdas.T, 2) * T), axis=0)
        chlorides = np.maximum(som, 0.0)
